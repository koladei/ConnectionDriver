...
//	const FLAG_               = 0x10000000; // unknown
	...
	function __construct($host, $user = null, $password = null, $port = 80, $isSecure = false, $domain='', $workstation='') {
		...
		// Extract the domain name from the username if the domain is not set
		$userData = explode('@',$user);
		if(isset($userData[1]) && $domain == '') {
			$domain = $userData[1];
		}
	...
	protected function _getResponse($hasBody = true) {
		...
		while(!feof($this->socket)) {
			...
			if($isHead) {
				...
				if($isHttpStatus) {
					...
					if($line == '') {
						...
						// If a particular header is encountered multiple times, convert it to an array.
			...
			//Response body
				...
				if($hasBody && is_null($contentLength) && $response['status'] == 200){
					
					// $response['body'] = '';
					// do {
						$line = stream_get_contents($this->socket, 297657);
						$response['body'] .= $line;
					// } while(!feof($this->socket));
					break;
				...
				} else if($contentLength == 0 || !$hasBody) {
					unset($response['body']);
					break;
				}
